...
def getUeberschuss():
    ...
    if len(dats1) > 0:
        if len(dats1[0])> 5 and len(dats1[1])>5:
            if dats1[0][5] == "Ok" and dats1[1][5] == "Ok":
                ueberschuss = - dats1[0][1] - dats1[1][1]   #negative Bilanz ist Entnahme
...
#
def setzeVerbraucher (zuVerbrauchen, cycle, battLaderFrei, aussentemp):
    ...
    if SOMMER:        
        if cycle > gHoldcycleFilterPumpe:
            ss, vv = setWithHyst(zuVerbrauchen - verbraucht, PACOffset+1000*filterPumpeVerbrauch, 100, 2, kellerschalter) #filter
            s+=ss
            verbraucht+=vv
            if vv > 0:
...
#fussboden:
    ...
    ss, vv = setWithHyst(zuVerbrauchen - verbraucht, PACOffset+1500, 100, int(3+((cycle/100+2)%3)), kellerschalter)
    s+=ss
    verbraucht+=vv
    ...
    try:
        ...
        while 1:
            ...
            if autoBoiler or autoBoden:
                ...
                warmwasserEtc = setzeVerbraucher (ueberschuss+warmwasserEtc, cycle, battLaderFrei, aussentemp)
            ...
            while 1:
                ...
                autoBoiler = myRead("VAR/AUTOBOILER", "kellerraspi:8000")
                autoBoden = myRead("VAR/AUTOBODEN", "kellerraspi:8000")
                if  autoBoiler or autoBoden:   # kann ich nun damit unterbrechen und manuell bedienen.
                    break
                allesAus()
                setMainMessage("auto - off")

                while not autoBoden and not autoBoiler:
